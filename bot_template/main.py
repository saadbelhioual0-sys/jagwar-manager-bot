...
# ========== APIs الجديدة التي أرسلتها ==========
# API 1: /mk و /stop_mk (بورت 8000)
MK_API_URL = "http://212.132.115.250:8000"

# API 2: /spam و /stop_spam (بورت 8001)
SPAM_API_URL = "http://212.132.115.250:8001"

# API 3: /ghost و /attack (بورت 8002)
GHOST_API_URL = "http://212.132.115.250:8002/api/ghost"
LAG_GHOST_API_URL = "http://212.132.115.250:8002/api/ghost_attack"

# API 4: /msg (بورت 8005 - عنوان مختلف)
MSG_API_URL = "http://91.99.5.210:8005"

# API 5: /friends (بورت 8005 - عنوان مختلف)
FRIENDS_API_URL = "http://212.132.115.250:8005/spam"

# API 6: /sp_clan (بورت 8006)
CLAN_API_URL = "http://212.132.115.250:8006/SpamClan"

# API 7: اللايكات (نفس العنوان القديم أو يمكنك تغييره)
LIKE_API_URL = "http://alliff-d5m-api-like.hf.space/like"
# ===============================================
...
# ========== دالة الـ LIKE المعدلة باستخدام الـ API الجديد ==========
def send_likes(uid):
    try:
        print(f"[DEBUG] Sending like request for UID: {uid}")
        likes_api_response = requests.get(
            f"{LIKE_API_URL}?uid={uid}",
            timeout=30
        )
...
async def handle_ghost_command(target_id, team_code, name, command_type="normal"):
    try:
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120)) as temp_session:
            async with temp_session.get(api_url, timeout=120) as resp:
...
[FFFFFF]⚡ Maintenance in progress""", 
                        ...
                        if check_cmd(inPuTMsG, 'debug'):
                            ...
                            debug_msg = f"[FF0000]✅ JAGWAR BOT ONLINE! UID: {current_uid}"
...
[FFFFFF]⚡ All services restored""", 
                        ...
                        # ========== أمر /attack (تم تحديثه لاستخدام API الجديد) ==========
                        if check_cmd(inPuTMsG, 'attack'):
                            ...
                            team_code = parts[1]
                            name = parts[2] if len(parts) > 2 else "Attacker"
                            
                            async def attack_api_task():
                                try:
                                    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120)) as temp_session:
                                        # استخدام API الجديد للهجوم
                                        async with temp_session.get(f"{LAG_GHOST_API_URL}?teamcode={team_code}&name={name}", timeout=120) as resp:
                                            result = await resp.json()
                                            return result
                                except Exception as e:
                            ...
                            attack_result = await attack_api_task()
                        ...
                        # ========== أمر /friends (تم تحديثه باستخدام API الجديد) ==========
                            ...
                            try:
                                ...
                                if target_uid.isdigit():
                                    async def friends_task(uid, chat_id, chat_type, target):
                                        try:
                                            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120)) as temp_session:
                                                # استخدام API الجديد للأصدقاء
                                                async with temp_session.get(f"{FRIENDS_API_URL}?uid={target}", timeout=120) as resp:
                                                    if resp.status == 200:
                                                        result = await resp.json()
                        ...
                        # ========== أمر /sp_clan (تم تحديثه باستخدام API الجديد) ==========
                            ...
                            try:
                                ...
                                if target_clan_id.isdigit():
                                    async def sp_clan_task(uid, chat_id, chat_type, target):
                                        try:
                                            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120)) as temp_session:
                                                # استخدام API الجديد للكلان
                                                async with temp_session.get(f"{CLAN_API_URL}?clan_id={target}", timeout=120) as resp:
                                                    if resp.status == 200:
                                                        result = await resp.json()
                                                        if result.get("status") == "success":
                                                            P = await SEndMsG(chat_type, "[B][C][00FF00]✅ DoNe", uid, chat_id, key, iv)
                        ...
                        # ========== أمر /mk (تم تحديثه باستخدام API الجديد بورت 8000) ==========
                            ...
                            async def mk_task(user_id, chat_id_param, chat_type_param, msg):
                                ...
                                try:
                                    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120)) as temp_session:
                                        # استخدام API الجديد للمك (بورت 8000)
                        ...
                        # ========== أمر /stop_mk (تم تحديثه باستخدام API الجديد بورت 8000) ==========
                            ...
                            async def stop_mk_task(user_id, chat_id_param, chat_type_param, msg):
                                ...
                                try:
                                    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120)) as temp_session:
                                        # استخدام API الجديد لإيقاف المك (بورت 8000)
                        ...
                        # ========== أمر /spam (تم تحديثه باستخدام API الجديد بورت 8001) ==========
                            ...
                            async def spam_task(user_id, chat_id_param, chat_type_param, msg):
                                ...
                                try:
                                    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120)) as temp_session:
                                        # استخدام API الجديد للإغراق العام (بورت 8001)
                        ...
                        # ========== أمر /stop_spam (تم تحديثه باستخدام API الجديد بورت 8001) ==========
                            ...
                            async def stop_spam_task(user_id, chat_id_param, chat_type_param, msg):
                                ...
                                try:
                                    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120)) as temp_session:
                                        # استخدام API الجديد لإيقاف الإغراق (بورت 8001)
                        ...
                        # ========== أمر /ghost (تم تحديثه باستخدام API الجديد بورت 8002) ==========
                            ...
                            async def ghost_task(user_id, chat_id_param, chat_type_param, msg):
                                ...
                                try:
                                    # استخدام API الجديد للجوست (بورت 8002)
                                    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120)) as temp_session:
                                        async with temp_session.get(f"{GHOST_API_URL}?teamcode={team_code}&name={name}", timeout=120) as resp:
                                            result = await resp.json()
                        ...
                        # ========== أمر /lag_ghost (تم تحديثه باستخدام API الجديد بورت 8002) ==========
                            ...
                            async def lag_ghost_task(user_id, chat_id_param, chat_type_param, msg):
                                ...
                                try:
                                    # استخدام API الجديد للاغ جوست (بورت 8002)
                                    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120)) as temp_session:
                                        async with temp_session.get(f"{LAG_GHOST_API_URL}?teamcode={team_code}&name={name}", timeout=120) as resp:
                                            result = await resp.json()
                        ...
                        # ========== أمر /msg (تم تحديثه باستخدام API الجديد) ==========
...
[FF0000]╠══════════╣
[FFD700]⚡ OWNER : [FFFFFF]JAGWAR    
[FFD700]✨ Name on Telegram  : [FFFFFF]@SOLO_JAGWAR  
[FF0000]╚══════════╝
[FFD700]✨ Developer —͟͞͞ </> JAGWAR ⚡"""
                                message = base_msg + f"""
...
[FFD700]✨ Developer —͟͞͞ </> JAGWAR ⚡"""
                        ...
                        if check_cmd(inPuTMsG, 'stop') and is_admin(current_uid):
                            ...
                            if len(parts) == 2:
                                ...
                                processing_msg = get_system_msg("squad", "processing")
                                P = await SEndMsG(current_chat_type, processing_msg, current_uid, current_chat_id, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                                ...
                                try:
                                    ...
                                    P = await SEndMsG(current_chat_type, get_system_msg("squad", "success"), current_uid, current_chat_id, key, iv)
                                    await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                                    continue
                                except Exception as e:
                                    P = await SEndMsG(current_chat_type, get_system_msg("squad", "error"), current_uid, current_chat_id, key, iv)
                            ...
                            try:
                                message1 = HELP_MSG_1 if HELP_MSG_1 else f"""[C][B][ADD8E6]═══JAGWAR BOT |
...
[AFEEEE]⚡ /attack [team_code] [name]

[AFEEEE]━━━━━━━━━━━━[E0FFFF]"""
                                
                                message2 = HELP_MSG_2 if HELP_MSG_2 else f"""[C][B][FFD700] JAGWAR BOT | [FFFF00][B]
...
async def MaiiiinE():
    ...
    os.system('clear')
    print(render('JAGWAR', colors=['white', 'green'], align='center'))
    print('')
